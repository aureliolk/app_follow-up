...
exports.parseDelayStringToMs = parseDelayStringToMs;
exports.formatMsToDelayString = formatMsToDelayString;
 ...
 */
function parseDelayStringToMs(delayString) {
 ...
 */
function formatMsToDelayString(ms) {
    ...
    if (numMs === weeks * factors.w && weeks > 0)
        return `${weeks}w`;
    if (numMs === days * factors.d && days > 0)
        return `${days}d`;
    if (numMs === hours * factors.h && hours > 0)
        return `${hours}h`;
    if (numMs === minutes * factors.m && minutes > 0)
        return `${minutes}m`;
