...
import { SearchBar, KanbanBoard, FollowUpDetailModal, Footer, ErrorMessage } from "../_components"
import MainNavigation from "../_components/MainNavigation"
import type { FollowUp } from "../_types"
...
export default function KanbanPage() {
  ...
  const filteredFollowUps = followUps.filter((followUp) => {
    ...
    if (followUp.metadata) {
      const metadata = typeof followUp.metadata === "string" ? JSON.parse(followUp.metadata) : followUp.metadata
  ...
  const handleCancelFollowUp = async (id: string) => {
    try {
      ...
      // Atualizar a lista local após o sucesso da API
      setFollowUps((prevFollowUps) =>
        prevFollowUps.map((followUp) => (followUp.id === id ? { ...followUp, status: "canceled" } : followUp)),
      )
