...
import { SearchBar, ErrorMessage, Footer, CampaignForm, MainNavigation } from "./_components/index"
...
export default function CampaignsPage() {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setCampaigns((prevCampaigns) =>
          prevCampaigns.map((c) => (c.id === campaign.id ? { ...c, active: !c.active } : c)),
        )
      }
    } catch (err) {
  ...
  const handleDeleteCampaign = async (campaign: Campaign) => {
    ...
    if (campaign.activeFollowUps > 0) {
      if (
        !confirm(
          `Esta campanha tem ${campaign.activeFollowUps} follow-ups ativos. Excluí-la pode afetar esses fluxos. Deseja continuar?`,
        )
      ) {
        return
      }
        ...
        <div className="mb-6">
          ...
          {isLoading ? (
                ...
                <tbody className="bg-gray-700 divide-y divide-gray-600">
                  {filteredCampaigns.map((campaign) => (
                    <tr key={campaign.id} className="hover:bg-gray-650">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{campaign.stepsCount || 0}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center space-x-3">
                        ...
                        >
                          <div
                            className={`relative inline-block w-10 h-5 transition-colors duration-200 ease-in-out rounded-full ${
                              campaign.active ? "bg-orange-600" : "bg-red-600"
                            }`}
                          >
                            <span
                              className={`absolute inset-0 flex items-center justify-${campaign.active ? "end" : "start"} w-4 h-4 pl-1`}
                            >
                              <span
                                className={`w-3 h-3 transition-colors duration-200 ease-in-out rounded-full bg-white transform ${
            ...
            <div className="text-center py-8 text-gray-400">
              ...
              {!searchTerm && <p className="mt-2">Crie sua primeira campanha clicando no botão "Nova Campanha".</p>}
            </div>
          )}
        </div>
      </main>
