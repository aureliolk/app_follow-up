...
import { funnelStepSchema, type FunnelStep, type FunnelStage } from "@/app/follow-up/_types/schema"
...
const StepFormRHF: React.FC<StepFormProps> = ({
  ...
  return (
    <form onSubmit={onSubmitHandler} id="step-form" className="bg-gray-700 p-4 rounded-lg">
      <h4 className="text-sm font-medium text-white mb-3">{isEditing ? "Editar Estágio" : "Adicionar Novo Estágio"}</h4>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          <label className="block text-xs font-medium text-gray-400 mb-1">Etapa do Funil *</label>
          <select
            {...register("stage_id")}
          ...
          {errors.stage_id && <p className="mt-1 text-xs text-red-500">{errors.stage_id.message}</p>}
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col space-y-2">
            ...
            />
            {errors.wait_time && <p className="mt-1 text-xs text-red-500">{errors.wait_time.message}</p>}
        ...
        <div>
          <div className="flex items-center">
            ...
            />
            <label className="ml-2 text-sm text-gray-300">Responder automaticamente</label>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-end space-x-3">
        ...
        <button
          ...
          disabled={isLoading}
          className={`px-4 py-2 ${
            isEditing ? "bg-blue-600 hover:bg-blue-700" : "bg-orange-600 hover:bg-orange-700"
