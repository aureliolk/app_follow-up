...
const CampaignForm: React.FC<CampaignFormProps> = ({
  ...
  useEffect(() => {
    ...
    if (initialData?.steps && Array.isArray(initialData.steps) && initialData.steps.length > 0) {
      ...
      const formattedSteps = initialData.steps
        .map((step: any) => {
  ...
  const handleRemoveStep = async (index: number) => {
    ...
    if (
      !confirm(
        `Tem certeza que deseja remover o estágio "${stepToRemove.template_name}" da etapa "${stepToRemove.stage_name}"?`,
      )
    ) {
      return
    }
  ...
  const handleRemoveFunnelStage = async (stageId: string) => {
    if (
      !confirm(
        "Tem certeza que deseja remover esta etapa do funil? Todos os estágios associados também serão removidos.",
      )
    ) {
      return
    }
        ...
        if (success) {
          ...
          const updatedSteps = steps.filter((step) => {
            const isRelatedToRemovedStage =
              step.stage_id === stageId || funnelStages.find((s) => s.id === stageId)?.name === step.stage_name
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-6">
      <h2 className="text-xl font-semibold text-white mb-4">{initialData?.id ? "Editar Campanha" : "Nova Campanha"}</h2>
      ...
      <div className="mb-6 mt-8">
        ...
        {(funnelStages.length > 0 || initialData?.id) && (
          <FunnelStageList stages={funnelStages} onEdit={handleEditFunnelStage} onRemove={handleRemoveFunnelStage} />
        )}
        ...
        {funnelStages.length === 0 && !initialData?.id && (
          <div className="bg-gray-700 p-4 rounded-lg mb-4 text-center">
            <p className="mb-4">Você está criando uma nova campanha.</p>
            <p className="italic text-sm text-gray-500 mb-4">
              Primeiro, crie a campanha com as informações básicas. Depois você poderá adicionar etapas e estágios
              específicos a ela.
            </p>
          </div>
        )}
        ...
        <div className="mb-6">
          ...
          {steps.length > 0 ? (
            <div className="bg-gray-700 rounded-lg overflow-hidden mb-4">
              <FunnelStagesTabs steps={steps} onRemoveStep={handleRemoveStep} onEditStep={handleEditStep} />
            </div>
          ) : (
