      ...
      <table className="min-w-full divide-y divide-gray-600">
        ...
        <tbody className="divide-y divide-gray-600">
          {followUps.map((followUp) => (
            <tr key={followUp.id} className="hover:bg-gray-650 cursor-pointer" onClick={() => onSelect(followUp)}>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-white">
                {followUp.client_id}
                {followUp.is_responsive && (
                  <span className="ml-2 px-2 py-0.5 text-xs rounded-full bg-purple-700 text-white">Responsivo</span>
                )}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{followUp.campaign.name}</td>
