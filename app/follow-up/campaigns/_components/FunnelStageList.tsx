                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-right">
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      ...
                      e.stopPropagation();
                      try {
                        if (typeof window !== 'undefined' && window.addStageStep) {
                          window.addStageStep(stage.id);
                        } else {
                          console.warn('Função addStageStep não está disponível');
                        }
                      } catch (err) {
                        console.error('Erro ao tentar adicionar estágio:', err);
                      }
                    }}
                    className="text-green-400 hover:text-green-300 mx-2"
                    title="Adicionar estágio nesta etapa"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
