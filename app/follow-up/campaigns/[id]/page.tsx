...
import { ErrorMessage, Footer, MainNavigation } from "../_components/index"
import Link from "next/link"
import followUpService from "../../_services/followUpService"
import type { Campaign, CampaignStep, FunnelStage } from "../../_types"
import CampaignFormHook from "../_components/CampaignFormHook"
  ...
  const handleRemoveFunnelStage = async (stageId: string): Promise<boolean> => {
    try {
      ...
      if (currentSteps.length > 0) {
        ...
        if (
          !confirm(`Esta etapa contém ${currentSteps.length} estágios. Todos eles serão removidos. Deseja continuar?`)
        ) {
          return false
        }
