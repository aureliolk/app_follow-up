...
import { Edit, Trash2, PlusCircle, Clock, AlertTriangle, Loader2, Info, GripVertical, Plus } from 'lucide-react';
...
// Tipo local que aceita datas como string ou Date E delay como string/number/bigint
type ApiFollowUpRule = Omit<PrismaRule, 'created_at' | 'updated_at' | 'delay_milliseconds'> & {
  created_at: string | Date;
  updated_at: string | Date;
  delay_milliseconds: string | number | bigint; // Ajustado
};
...
export default function AiFollowUpRules() {
  ...
  const [editingRule, setEditingRule] = useState<ApiFollowUpRule | null>(null);
  ...
  const handleOpenEditForm = (rule: ApiFollowUpRule) => {
    setEditingRule(rule);
    setFormData({
