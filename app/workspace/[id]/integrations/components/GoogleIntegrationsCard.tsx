...
import { updateGoogleCalendarConversionAction } from '@/lib/actions/workspaceSettingsActions';

export default function GoogleIntegrationsCard() {
  const { workspace, isLoading: workspaceLoading, refreshWorkspaces, updateCurrentWorkspaceField } = useWorkspace();
  const router = useRouter();
  const [isConnecting, setIsConnecting] = useState(false);
  const [isDisconnecting, setIsDisconnecting] = useState(false);
  const [isUpdatingConversion, startTransition] = useTransition();
  ...
  const handleToggleConversion = (enabled: boolean) => {
    ...
    // Atualiza a UI otimisticamente ANTES de chamar a action
    updateCurrentWorkspaceField('google_calendar_event_conversion_enabled', enabled);

    startTransition(async () => {
      const result = await updateGoogleCalendarConversionAction({ workspaceId: workspace.id, enabled });
      if (result.success) {
        toast.success('Configuração de conversão atualizada!');
      } else {
        toast.error(`Falha ao atualizar: ${result.error}`);
        // Reverte a UI se a action falhar
        updateCurrentWorkspaceField('google_calendar_event_conversion_enabled', !enabled);
      }
    });
  };
        ...
        {workspaceLoading && !workspace ? (
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-3 sm:gap-4 p-4 border border-border rounded-md bg-background/30">
            <div className='flex flex-col'>
              ...
              {/* Checkbox de Conversão de Follow-up */}
              <div className="flex items-center space-x-2 pt-3">
                {isUpdatingConversion && <Loader2 className="h-4 w-4 animate-spin mr-1" />}
                <Checkbox
                  id="google-conversion-toggle"
                  checked={!!workspace.google_calendar_event_conversion_enabled}
                  onCheckedChange={(checked) => {
                    handleToggleConversion(Boolean(checked));
                  }}
                  disabled={isUpdatingConversion}
                />
                <Label htmlFor="google-conversion-toggle" className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                  Converter Follow-up ao agendar evento no Google Calendar
                </Label>
              </div>
            </div>
