...
export default function WhatsappSettingsForm({ currentSettings }: WhatsappSettingsFormProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!phoneNumberId || !businessAccountId || !accessToken || !appSecret || !verifyToken) {
    ...
    try {
       console.log("Enviando para Action:", {
          ...
          phoneNumberId,
          businessAccountId,
          accessToken, // Enviando o token real
          appSecret,   // Enviando o segredo real
      ...
      const result = await saveWhatsappCredentialsAction({
        ...
        phoneNumberId,
        businessAccountId,
        accessToken, // Action deve criptografar
        appSecret,   // Action deve criptografar
        webhookVerifyToken: verifyToken,
      });
          ...
          <Label htmlFor="verifyToken">Token de Verificação do Webhook</Label>
           <div className="flex items-center space-x-2">
              <Input
                ...
                placeholder="Token para verificar seu webhook"
                required
                disabled={isLoading}
              />
              <Button type="button" variant="outline" onClick={generateVerifyToken} disabled={isLoading}>Gerar</Button>
           </div>
          <p className="text-[0.8rem] text-muted-foreground">Crie um token seguro ou gere um. Você usará este token na configuração do webhook na Meta.</p>
        </div>
      </div>
