...
export default async function WhatsappIntegrationPage({ params }: WhatsappIntegrationPageProps) {
  ...
  const appBaseUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://SUA_URL_BASE'; 
  // Construir URL dinâmica do webhook
  const webhookUrl = settings.whatsappWebhookRouteToken 
    ? `${appBaseUrl}/api/webhook/ingress/whatsapp/${settings.whatsappWebhookRouteToken}`
        ...
        <CardContent>
          <div className="mb-6 p-4 border rounded-md bg-muted/50">
             ...
             <div className="space-y-1">
               {webhookUrl ? (
                 <p className="text-sm"><strong>URL de Callback:</strong> <code className="bg-gray-200 dark:bg-gray-700 px-1 py-0.5 rounded text-xs">{webhookUrl}</code></p>
               ) : (
                 <p className="text-sm text-orange-600 dark:text-orange-400"><strong>URL de Callback:</strong> Salve as configurações primeiro para gerar a URL.</p>
               )}
               <p className="text-sm"><strong>Token de Verificação:</strong> <code className="bg-gray-200 dark:bg-gray-700 px-1 py-0.5 rounded text-xs">{settings.webhookVerifyToken || '[Será gerado/mostrado após salvar]'}</code></p>
               <p className="text-xs text-muted-foreground mt-1">Assine pelo menos o evento 'messages' nos campos do webhook.</p>
             </div>
          </div>
