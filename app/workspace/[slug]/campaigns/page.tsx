'use client';

import { useWorkspace } from '@/context/workspace-context';
import { useEffect, useState } from 'react';
import { followUpService } from '@/app/follow-up/_services/followUpService';
import { Loader2, ArrowUpRight, PlusCircle, Edit, Trash, Check, X } from 'lucide-react';
import Link from 'next/link';
import { useRouter } from 'next/navigation';

export default function WorkspaceCampaigns() {
  const { workspace, isLoading } = useWorkspace();
  const [campaigns, setCampaigns] = useState<any[]>([]);
  const [loadingData, setLoadingData] = useState(true);
  const router = useRouter();

  useEffect(() => {
    async function loadCampaignData() {
      if (!workspace) return;
      
      try {
        setLoadingData(true);
        
        // Carregar campanhas do workspace
        const workspaceCampaigns = await followUpService.getCampaigns(workspace.id);
        setCampaigns(workspaceCampaigns);
      } catch (error) {
        console.error('Erro ao carregar campanhas:', error);
      } finally {
        setLoadingData(false);
      }
    }
    
    loadCampaignData();
  }, [workspace]);

  // Função para redirecionar para a página de criação de campanha
  const handleNewCampaign = () => {
    if (workspace) {
      sessionStorage.setItem('activeWorkspaceId', workspace.id);
      router.push('/follow-up/campaigns');
    }
  };
  
  // Função para editar uma campanha existente
  const handleEditCampaign = (campaignId: string) => {
    if (workspace) {
      sessionStorage.setItem('activeWorkspaceId', workspace.id);
      router.push(`/follow-up/campaigns/${campaignId}`);
    }
  };

  if (isLoading || loadingData) {
    return (
      <div className="flex items-center justify-center h-full">
        <Loader2 className="h-8 w-8 animate-spin text-[#F54900]" />
      </div>
    );
  }

  if (!workspace) return null;

  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-semibold text-white">Campanhas de Follow-up</h1>
        
        <button
          onClick={handleNewCampaign}
          className="flex items-center gap-1 px-4 py-2 rounded-md bg-[#F54900] text-white hover:bg-[#d43d00] transition-colors"
        >
          <PlusCircle className="h-4 w-4" />
          <span>Nova Campanha</span>
        </button>
      </div>
      
      {/* Estatísticas */}
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        <div className="bg-[#111111] border border-[#333333] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-400 mb-1">Total de Campanhas</h3>
          <p className="text-2xl font-bold text-white">{campaigns.length}</p>
        </div>
        
        <div className="bg-[#111111] border border-[#333333] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-400 mb-1">Campanhas Ativas</h3>
          <p className="text-2xl font-bold text-white">{campaigns.filter(c => c.active).length}</p>
        </div>
        
        <div className="bg-[#111111] border border-[#333333] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-400 mb-1">Total de Etapas</h3>
          <p className="text-2xl font-bold text-white">{campaigns.reduce((total, campaign) => total + (campaign.stepsCount || 0), 0)}</p>
        </div>
        
        <div className="bg-[#111111] border border-[#333333] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-400 mb-1">Follow-ups Ativos</h3>
          <p className="text-2xl font-bold text-white">{campaigns.reduce((total, campaign) => total + (campaign.activeFollowUps || 0), 0)}</p>
        </div>
      </div>
      
      {/* Lista de Campanhas */}
      <div className="bg-[#111111] border border-[#333333] rounded-lg overflow-hidden">
        <div className="p-4 border-b border-[#333333]">
          <h2 className="font-medium text-white">Todas as Campanhas</h2>
        </div>
        
        <div className="overflow-x-auto">
          {campaigns.length > 0 ? (
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-gray-400 uppercase bg-[#0a0a0a]">
                <tr>
                  <th className="px-4 py-3">Nome</th>
                  <th className="px-4 py-3">Descrição</th>
                  <th className="px-4 py-3">Status</th>
                  <th className="px-4 py-3">Etapas</th>
                  <th className="px-4 py-3">Follow-ups Ativos</th>
                  <th className="px-4 py-3">Criada em</th>
                  <th className="px-4 py-3">Ações</th>
                </tr>
              </thead>
              <tbody>
                {campaigns.map((campaign) => (
                  <tr key={campaign.id} className="border-b border-[#333333] hover:bg-[#1a1a1a]">
                    <td className="px-4 py-3 text-white font-medium">{campaign.name}</td>
                    <td className="px-4 py-3 text-gray-300">{campaign.description || '-'}</td>
                    <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs ${
                        campaign.active ? 'bg-green-900 text-green-300' : 'bg-red-900 text-red-300'
                      }`}>
                        {campaign.active ? 'Ativa' : 'Inativa'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-300">{campaign.stepsCount || 0}</td>
                    <td className="px-4 py-3 text-gray-300">{campaign.activeFollowUps || 0}</td>
                    <td className="px-4 py-3 text-gray-300">
                      {new Date(campaign.created_at).toLocaleDateString()}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleEditCampaign(campaign.id)}
                          className="text-blue-400 hover:text-blue-300"
                          title="Editar campanha"
                        >
                          <Edit className="h-4 w-4" />
                        </button>
                        
                        <button
                          className={`${campaign.active ? 'text-red-400 hover:text-red-300' : 'text-green-400 hover:text-green-300'}`}
                          title={campaign.active ? 'Desativar campanha' : 'Ativar campanha'}
                        >
                          {campaign.active ? <X className="h-4 w-4" /> : <Check className="h-4 w-4" />}
                        </button>
                      </div>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          ) : (
            <div className="p-6 text-center text-gray-400">
              <p>Nenhuma campanha encontrada para este workspace.</p>
              <button
                onClick={handleNewCampaign}
                className="mt-2 text-[#F54900] hover:underline"
              >
                Criar a primeira campanha
              </button>
            </div>
          )}
        </div>
      </div>
      
      {/* Link para retornar ao workspace */}
      <div className="mt-6 text-center">
        <Link 
          href={`/workspace/${workspace.slug}`} 
          className="text-gray-400 hover:text-white text-sm"
        >
          Voltar para o dashboard do workspace
        </Link>
      </div>
    </div>
  );
}