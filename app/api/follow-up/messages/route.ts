...
export async function GET(req: NextRequest) {
  try {
    ...
    // Validação
    if (!followUpId) {
      return NextResponse.json({ success: false, error: "followUpId é obrigatório" }, { status: 400 })
    }
    ...
    return NextResponse.json({ success: false, error: error.message || "Erro ao buscar mensagens" }, { status: 500 })
  }
}
