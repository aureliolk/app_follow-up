...
export async function POST(req: NextRequest) {
  try {
    ...
    const { funnel_stage_id, name, template_name, wait_time, message_content, message_category, auto_respond } = body
    ...
    const stepData = {
      ...
      auto_respond: auto_respond !== undefined ? auto_respond : false,
    }
    ...
    if (createError) {
      console.error("Erro detalhado ao criar passo:", JSON.stringify(createError, null, 2));
      throw createError;
    }
    ...
    console.error("Erro detalhado (stringified):", JSON.stringify(error, null, 2))
    return NextResponse.json(
      {
        success: false,
        error: "Erro interno do servidor",
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { id, funnel_stage_id, name, template_name, wait_time, message_content, message_category, auto_respond } =
      body
