...
async function processFollowUp(followUpId) {
  try {
    ...
    console.log(
      `Hora de conclusão: ${formatDateTime(finalFollowUp.completed_at ? new Date(finalFollowUp.completed_at) : new Date())}`,
    )
  } catch (error) {
...
export { processFollowUp, createAndProcessFollowUp, simulateMessageSend, simulateClientResponse, parseTimeString }
