...
async function waitForStageChange(followUpId, expectedStageName, timeout = CONFIG.timeout) {
  ...
  while (Date.now() - startTime < timeout) {
    try {
      ...
      try {
        if (status.metadata) {
          const metadata = typeof status.metadata === "string" ? JSON.parse(status.metadata) : status.metadata
...
async function testCampaignFlow(campaignId, clientId) {
  ...
  try {
    ...
    while (currentStageIndex < stageNames.length) {
      ...
      log(
        `\n===> PROCESSANDO ETAPA: ${currentStageName} (${currentStageIndex + 1}/${stageNames.length})`,
