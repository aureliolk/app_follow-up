// scripts/reset-followup-tables.js
// Script para deletar e recriar as tabelas de follow-up
...
// Criar interface para perguntar ao usuário
...
async function resetFollowUpTables() {
  try {
    console.log('🔄 Preparando para redefinir tabelas de follow-up...');
    
    const isConfirmed = await confirm('⚠️ ATENÇÃO: Isso irá APAGAR todos os dados de follow-up. Deseja continuar?');
    ...
    // 5. Remover colunas adicionais
    console.log('5. Removendo colunas adicionais...');
    await prisma.$executeRawUnsafe(`
      ALTER TABLE "follow_up_schema"."follow_ups" 
      DROP COLUMN IF EXISTS "current_stage_name",
      DROP COLUMN IF EXISTS "previous_stage_name",
      DROP COLUMN IF EXISTS "waiting_for_response",
      DROP COLUMN IF EXISTS "paused_reason",
      DROP COLUMN IF EXISTS "last_response",
      DROP COLUMN IF EXISTS "last_response_date",
      DROP COLUMN IF EXISTS "processed_by_response";
    `);
    
    // 6. Criar novas tabelas e campos
    ...
    await prisma.$executeRawUnsafe(`
      CREATE INDEX "follow_up_current_stage_name_idx" ON "follow_up_schema"."follow_ups"("current_stage_name");
      CREATE INDEX "follow_up_waiting_for_response_idx" ON "follow_up_schema"."follow_ups"("waiting_for_response");
      CREATE INDEX "follow_up_state_transitions_follow_up_id_idx" ON "follow_up_schema"."follow_up_state_transitions"("follow_up_id");
      CREATE INDEX "follow_up_client_responses_follow_up_id_idx" ON "follow_up_schema"."follow_up_client_responses"("follow_up_id");
    `);
...
async function main() {
  try {
    ...
    if (success) {
      ...
      console.log('\n====================================================');
      console.log('❌ REDEFINIÇÃO CANCELADA');
      console.log('====================================================\n');
    }
  } catch (error) {
    console.error('\n====================================================');
